...
<div class="row" style="
   ...
   <div class="col-md-3">
      <a class="btn btn-dark" href="{{ route('active.properties') }}">Add a Property Category</a>
   </div>
   <div class="col-md-3">
...
@section('activepropertiesmgmtscript')
  <script>
    ...
    var activerentalpropertiestable = $('#activatedpropertiestable').DataTable({
      ...
      //   show editing modal
   $('body').on('click','.editpropertydetails',function(){
      ...
      $.ajax({
         ...
         {
            console.log(response.editpropertydetails.property_details);
            ...
            else if(response.status==200)
            {
               ...
               $('.propertyhsevideo').val(response.editpropertydetails.property_video);
      ...
      $(document).on('submit','.updatepropertydetails',function(e)
         {
            e.preventDefault();
            ...
            var form = $('#updatepropertyform')[0];
            var formpropertydata=new FormData(form);
            $.ajax({
               url:updatepropertyurl,
               method:'POST',
               processData:false,
               contentType:false,
               data:formpropertydata,
               success:function(response)
            {
               ...
               } else if (response.status==200)
               {
                     ...
                     // preview an image that was previously uploaded
                     $('.sellpropertyimage').val('');
                     $('.propertyimg').html(showpropertyimage);
                     // preview an image that was previously uploaded
                     var deletepropertyimage=$('#showpropertyimage').removeAttr('src');
                     $('.sellpropertyimage').html('deletepropertyimage');

                     $('.propertyhsevideo').val('');
